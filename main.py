...
img = cv.imread('/Users/delouya.thefist/PycharmProjects/imageClassificationAI/ai training pics/horse.jpg')
img = cv.cvtColor(img, cv.COLOR_BGR2RGB)  # convert the image to RGB
img = cv.resize(img, (32, 32))  # resize the image to 32x32

plt.imshow(img, cmap=plt.cm.binary)  # show the image

prediction = model.predict(np.array([img]))  # predict the class of the image
index = np.argmax(prediction)  # get the index of the class with the highest probability
print("Prediction is:", class_names[index])  # print the prediction
